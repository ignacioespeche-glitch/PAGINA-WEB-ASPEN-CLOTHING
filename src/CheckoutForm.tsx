...
export const CheckoutForm = () => {
  ...
  const obtenerLinkWhatsAppEfectivo = () => {
    ...
    return `https://wa.me/${telefonoLocal}?text=${encodeURIComponent(`Hola Aspen! Necesito el cupón / QR para pagar en Rapipago o Pago Fácil.\n\nMis datos son:\n• Nombre: ${nombreCliente}\n• Total neto (con envío): $${totalPedido},00`)}`;
  };
  ...
  if (compraExitosa) {
    ...
    return (
      <div style={{ padding: '160px max(4vw, 20px) 80px max(4vw, 20px)', minHeight: '75vh', fontFamily: 'Inter, sans-serif', maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
    ...
    <div className="checkout-container" style={{ padding: '140px max(4vw, 20px) 40px max(4vw, 20px)', minHeight: '80vh', fontFamily: 'Inter, sans-serif' }}>
      <form onSubmit={handlePagarAhoraSubmit} style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '80px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
            ...
            <div style={{ marginTop: '4px' }}>
              {metodoPago === 'tarjeta' && (
                <div style={{ border: '1px solid #000', padding: '32px 24px', backgroundColor: '#fff', display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center', textAlign: 'center' }}>
                  <button
                    ...
                    style={{ width: '100%', background: '#000', color: '#fff', border: '1px solid #000', padding: '16px', fontWeight: 700, fontSize: '11px', letterSpacing: '2px', cursor: 'pointer', textTransform: 'uppercase' }}
                  >
                    {cargandoPasarela ? 'REDIRECCIONANDO A PASARELA...' : 'PAGO NUBE SEGURO'}
                  </button>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <p style={{ margin: 0, fontSize: '12px', color: '#000', fontWeight: 700, letterSpacing: '0.5px' }}>🔒 PagoNube es 100% seguro</p>
                    <p style={{ margin: 0, fontSize: '11px', color: '#666', lineHeight: '1.5', maxWidth: '400px' }}>Al iniciar la compra, serás redirigido bajo un entorno encriptado y oficial certificado por Tiendanube para colocar los datos de tu tarjeta con total resguardo bancario.</p>
                  </div>
                </div>
              )}
              ...
              {metodoPago === 'efectivo' && (
                <div style={{ padding: '24px', border: '1px solid #000', backgroundColor: '#fff', fontSize: '12px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <p style={{ margin: 0, color: '#333', lineHeight: '1.6' }}>Al confirmar tu pedido abajo, la app registrará la compra y abrirá la línea de WhatsApp corporativa para despacharte el código de barra o QR por el monto de <strong>${montoFinalAMostrar.toLocaleString('es-AR')},00</strong>.</p>
                </div>
              )}
            </div>
          </div>
          ...
          {errorPasarela && (
            <span style={{ fontSize: '10px', fontWeight: 700, color: '#DC2626', letterSpacing: '1px', textTransform: 'uppercase', display: 'block' }}>✕ {errorPasarela}</span>
          )}
          ...
          {metodoPago !== 'tarjeta' && (
            <button 
              type="submit" 
              disabled={cargandoPasarela}
              style={{ width: '100%', background: cargandoPasarela ? '#666' : '#000', color: '#fff', border: 'none', padding: '18px', fontWeight: 700, fontSize: '12px', letterSpacing: '2px', cursor: 'pointer', textTransform: 'uppercase' }}
            >
        ...
        <div style={{ background: 'transparent', padding: '35px 0 max(4vw, 20px) 0', border: 'none' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '25px', borderBottom: '1px solid #000', paddingBottom: '25px' }}>
            {carrito.map((item, index) => {
              if (!item) return null;
              const precioAMostrarPorItem = metodoPago === 'tarjeta' ? Math.round((item.precio || 0) * 1.20) : (item.precio || 0);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '25px', borderBottom: '1px solid #000', paddingBottom: '25px' }}>
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button type="button" onClick={handleAplicarCupon} style={{ background: '#000', color: '#fff', border: 'none', padding: '0 24px', fontSize: '11px', fontWeight: 600, letterSpacing: '1px', cursor: 'pointer', textTransform: 'uppercase' }}>APLICAR</button>
            </div>
